...
# 1. LOGLAMA (PROFESYONEL SEVİYE)
logging.basicConfig(
    format='%(asctime)s - %(levelname)s - %(message)s', 
    level=logging.INFO
)

# 2. YENİ TOKEN (GÜVENLİ)
BOT_TOKEN = "8674782699:AAFfax8p4YIcyYdu7DH6LhuPDlJCw5ALTV4"
leaderboard = {}

# 3. İYUK TAM SORU SETİ (25/25) - HİÇBİR KISALTMADAN
IYUK_SORU_SETI = [
...
# 4. PROFESYONEL MEKANİZMA (TAKILMA VE TEKRAR ENGELLEYİCİ)
...
async def start_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏳ *Yeni Sistem (İYUK) Yükleniyor...*\nEski emirler temizlendi. 15 saniye bekleyin!", parse_mode="Markdown")
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    quiz = context.user_data.get("quiz")
    if not quiz or not quiz["active"] or quiz["answered"]: return

    quiz["answered"] = True # İkinci cevap gelmesini engelle
    if update.poll_answer.option_ids[0] == quiz["questions"][quiz["current"]]["correct_option_id"]:
        quiz["score"] += 1

    quiz["current"] += 1
    await asyncio.sleep(3) # Spam koruması
    ...
    if quiz["current"] < 25:
        ...
        txt = "🏆 *İYUK FINAL GAME ŞAMPİYONLARI*\n\n"
        for i, p in enumerate(sorted_board[:10], 1):
            txt += f"{i}. {p['name']} - ✅ {p['score']}/25 ({p['sure']}sn)\n"
        
        await context.bot.send_message(quiz["chat_id"], txt, parse_mode="Markdown")
        quiz["active"] = False
...
def main():
    ...
    print("Bot yeni token ile açılıyor. Eski takılmalar imha ediliyor...")
