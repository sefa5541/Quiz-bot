...
# 1. GÜVENLİK VE LOG SİSTEMİ
logging.basicConfig(
    format='%(asctime)s - %(levelname)s - %(message)s', 
    level=logging.INFO
)

# 2. SENİN YENİ TOKENIN (DİREKT İÇİNE GÖMDÜM)
BOT_TOKEN = "8674782699:AAFfax8p4YIcyYdu7DH6LhuPDlJCw5ALTV4"
leaderboard = {}

# 3. İDARİ YARGILAMA (İYUK) TAM SORU SETİ
IYUK_SORU_SETI = [
...
async def start_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⚖️ *İYUK FINAL GAME BAŞLIYOR...*\n10 saniye sonra ilk soru gelecek!", parse_mode="Markdown")
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data or not data.get("active") or data.get("answered"):
        return
    ...
    if data["current"] < len(data["questions"]):
        ...
        txt = "🏆 *FINAL GAME (İYUK) LİDERLİK TABLOSU*\n\n"
...
def main():
    ...
    print("Bot yayına girdi. İYUK operasyonu hazır!")
